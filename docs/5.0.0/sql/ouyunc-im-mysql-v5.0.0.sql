...
CREATE TABLE `ouyunc_im_app`  (
  ...
  UNIQUE INDEX `appKey`(`app_key`) USING BTREE COMMENT '唯一索引'
...
CREATE TABLE `ouyunc_im_blacklist`  (
  ...
  `app_key` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '平台唯一标识',
...
CREATE TABLE `ouyunc_im_message`  (
  ...
  `content` varchar(5000) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '消息内容',
...
-- ----------------------------
-- Table structure for ouyunc_mqtt_topic
-- ----------------------------
DROP TABLE IF EXISTS `ouyunc_mqtt_topic`;
CREATE TABLE `ouyunc_mqtt_topic`  (
  `id` bigint NOT NULL COMMENT '主键id',
  `app_key` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '平台唯一标识',
  `topic` varchar(1000) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '主题',
  `description` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '主题描述',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `deleted` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否删除：1-已删除，0-未删除',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = 'mqtt 主题表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Table structure for ouyunc_mqtt_topic_subscribe
-- ----------------------------
DROP TABLE IF EXISTS `ouyunc_mqtt_topic_subscribe`;
CREATE TABLE `ouyunc_mqtt_topic_subscribe`  (
  `id` bigint NOT NULL COMMENT '主键id',
  `app_key` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '平台唯一标识',
  `topic_id` bigint NOT NULL COMMENT '主题id',
  `client_id` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '客户端id',
  `qos` tinyint(1) NULL DEFAULT NULL COMMENT 'qos 消息质量等级： 0,1,2',
  `no_local` tinyint(1) NULL DEFAULT NULL COMMENT '在 MQTT v3.1.1 中，如果你订阅了自己发布消息的主题，那么你将收到自己发布的所有消息。\r\n\r\n而在 MQTT v5 中，如果你在订阅时将此选项设置为 1，那么服务端将不会向你转发你自己发布的消息;0-false, 1-true',
  `retain_as_published` tinyint(1) NULL DEFAULT NULL COMMENT '这一选项用来指定服务端向客户端转发消息时是否要保留其中的 RETAIN 标识，注意这一选项不会影响保留消息中的 RETAIN 标识。因此当 Retain As Publish 选项被设置为 0 时，客户端直接依靠消息中的 RETAIN 标识来区分这是一个正常的转发消息还是一个保留消息，而不是去判断消息是否是自己订阅后收到的第一个消息（转发消息甚至可能会先于保留消息被发送，视不同 Broker 的具体实现而定）0-false, 1-true',
  `retain_handling` tinyint(1) NULL DEFAULT NULL COMMENT ' 这一选项用来指定订阅建立时服务端是否向客户端发送保留消息：\r\n\r\nRetain Handling 等于 0，只要客户端订阅成功，服务端就发送保留消息。\r\nRetain Handling 等于 1，客户端订阅成功且该订阅此前不存在，服务端才发送保留消息。毕竟有些时候客户端重新发起订阅可能只是为了改变一下 QoS，并不意味着它想再次接收保留消息。\r\nRetain Handling 等于 2，即便客户订阅成功，服务端也不会发送保留消息; ',
  `create_time` datetime NULL DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime NULL DEFAULT NULL COMMENT '修改时间',
  `deleted` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否删除：0-未删除，1-已删除',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = 'mqtt 主题订阅表' ROW_FORMAT = Dynamic;
