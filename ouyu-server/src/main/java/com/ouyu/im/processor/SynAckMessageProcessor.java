...
public class SynAckMessageProcessor extends AbstractMessageProcessor{
    ...
    public void doProcess(ChannelHandlerContext ctx, Packet packet) {
        ...
        HeartBeatMessage message = (HeartBeatMessage) packet.getMessage();
        // 需要判断收到的消息是目的地是否是本服务器，如果不是在次将消息包传递出去，如果是则处理
        final String synAck = message.getContent();
        final String remoteServerAddressStr = message.getFrom();
        ...
        if (ImConstant.SYN.equals(synAck)) {
            ...
            message.setContent(ImConstant.ACK);
            // 获取本地服务地址
            // 这里是发送消息的起点
            message.setFrom(IMContext.LOCAL_ADDRESS);
            message.setTo(remoteServerAddressStr);
            message.setTargetServerAddress(remoteServerAddressStr);
        ...
        if (ImConstant.ACK.equals(synAck)) {
            ...
            final String targetServerAddress = message.getTargetServerAddress();
            if (IMContext.LOCAL_ADDRESS.equals(targetServerAddress)) {
                // 清空missAckTimes 次数
                IMContext.MISS_ACK_TIMES_CACHE.invalidate(remoteServerAddress);
                // 如果相等则添加到注册表，清空路由信息，其实这里不需要清空了（不向外转发）
                //log.info("接收到远端IM服务：{}的ACK回应,现将该服务加入注册表中", remoteServerAddressStr);
                IMContext.CLUSTER_SERVER_REGISTRY_TABLE.get(remoteServerAddress, k -> IMClientPool.singleClientChannelPoolMap.get(remoteServerAddress));
                return;
            }
            // 不相等则继续传递
