...
//
    ...
    public MqttDecoderUtil() {
      this(DEFAULT_MAX_BYTES_IN_MESSAGE, DEFAULT_MAX_CLIENT_ID_LENGTH);
    }

    public MqttDecoderUtil(int maxBytesInMessage) {
    ...
    public MqttMessage doDecode(MqttVersion mqttVersion, ByteBuf buffer) {
     ...
     */
    private static MqttFixedHeader decodeFixedHeader(MqttVersion mqttVersion, ByteBuf buffer) {
        ...
        return validateFixedHeader(mqttVersion, resetUnusedFields(decodedFixedHeader));
    }
     ...
     */
    private Result<?> decodeVariableHeader(MqttVersion mqttVersion, ByteBuf buffer, MqttFixedHeader mqttFixedHeader, int bytesRemainingInVariablePart) {
        switch (mqttFixedHeader.messageType()) {
            case CONNECT:
                return decodeConnectionVariableHeader(buffer);

            case CONNACK:
                return decodeConnAckVariableHeader(mqttVersion, buffer);

            case UNSUBSCRIBE:
            case SUBSCRIBE:
            case SUBACK:
            case UNSUBACK:
                return decodeMessageIdAndPropertiesVariableHeader(mqttVersion, buffer);
            ...
            case PUBLISH:
                return decodePublishVariableHeader(mqttVersion, buffer, mqttFixedHeader);
    ...
    private Result<MqttPubReplyMessageVariableHeader> decodePubReplyMessage(ByteBuf buffer, int bytesRemainingInVariablePart) {
        final int packetId = decodeMessageId(buffer);
