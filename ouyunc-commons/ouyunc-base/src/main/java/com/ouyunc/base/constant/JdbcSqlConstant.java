...
public class JdbcSqlConstant {
    ...
    public enum MYSQL{
        ...
        DELETE_MQ_OUTBOX("DELETE FROM ouyunc_im_mq_outbox WHERE id = :id", "MQ 补发成功后删除 Outbox"),

        UPDATE_MQ_OUTBOX_RETRY("UPDATE ouyunc_im_mq_outbox SET status = :status, retry_count = :retry_count, next_retry_at = :next_retry_at, last_error = :last_error, update_time = CURRENT_TIMESTAMP WHERE id = :id", "Outbox 重试退避或置死信"),

        RESET_STALE_MQ_OUTBOX_SENDING("UPDATE ouyunc_im_mq_outbox SET status = :pending_status, update_time = CURRENT_TIMESTAMP WHERE status = :sending_status AND update_time < :stale_before", "回收超时 SENDING")
        ;
    ...
    public enum ORACLE {
        ...
        INSERT_MQ_OUTBOX("MERGE INTO OUYUNC_IM_MQ_OUTBOX t USING (SELECT :id AS ID, :topic AS TOPIC, :mq_key AS MQ_KEY, :biz_key AS BIZ_KEY, :packet_id AS PACKET_ID, :payload AS PAYLOAD, :status AS STATUS, :retry_count AS RETRY_COUNT, :next_retry_at AS NEXT_RETRY_AT, :last_error AS LAST_ERROR, :failure_context AS FAILURE_CONTEXT FROM DUAL) s ON (t.TOPIC = s.TOPIC AND t.BIZ_KEY = s.BIZ_KEY) WHEN MATCHED THEN UPDATE SET t.LAST_ERROR = s.LAST_ERROR, t.FAILURE_CONTEXT = s.FAILURE_CONTEXT, t.PAYLOAD = s.PAYLOAD, t.STATUS = s.STATUS, t.RETRY_COUNT = s.RETRY_COUNT, t.NEXT_RETRY_AT = s.NEXT_RETRY_AT, t.PACKET_ID = s.PACKET_ID, t.MQ_KEY = s.MQ_KEY, t.UPDATE_TIME = SYSTIMESTAMP WHEN NOT MATCHED THEN INSERT (ID, TOPIC, MQ_KEY, BIZ_KEY, PACKET_ID, PAYLOAD, STATUS, RETRY_COUNT, NEXT_RETRY_AT, LAST_ERROR, FAILURE_CONTEXT) VALUES (s.ID, s.TOPIC, s.MQ_KEY, s.BIZ_KEY, s.PACKET_ID, s.PAYLOAD, s.STATUS, s.RETRY_COUNT, s.NEXT_RETRY_AT, s.LAST_ERROR, s.FAILURE_CONTEXT)", "写入/刷新 MQ Outbox（冲突时复活为 PENDING）"),
        ...
        UPDATE_MQ_OUTBOX_RETRY("UPDATE OUYUNC_IM_MQ_OUTBOX SET STATUS = :status, RETRY_COUNT = :retry_count, NEXT_RETRY_AT = :next_retry_at, LAST_ERROR = :last_error, UPDATE_TIME = SYSTIMESTAMP WHERE ID = :id", "Outbox 重试退避或置死信"),

        RESET_STALE_MQ_OUTBOX_SENDING("UPDATE OUYUNC_IM_MQ_OUTBOX SET STATUS = :pending_status, UPDATE_TIME = SYSTIMESTAMP WHERE STATUS = :sending_status AND UPDATE_TIME < :stale_before", "回收超时 SENDING")
        ;
