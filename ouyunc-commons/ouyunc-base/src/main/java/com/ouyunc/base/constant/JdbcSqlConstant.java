...
public class JdbcSqlConstant {
    ...
    public enum MYSQL{
        ...
        SELECT_ALL_FRIEND("SELECT id, user_id, friend_user_code, friend_user_id, friend_nick_name, shield, way, channel, join_time, create_time, update_time FROM ouyunc_im_friend f where f.user_id = :user_id and exists (select 1 from ouyunc_im_user u where u.id = f.user_id and u.app_key = :app_key and u.del_flag = 0)", "按租户查询用户全部好友"),
    ...
    public enum ORACLE {
        ...
        SELECT_ALL_FRIEND("SELECT ID, USER_ID, FRIEND_USER_CODE, FRIEND_USER_ID, FRIEND_NICK_NAME, SHIELD, WAY, CHANNEL, JOIN_TIME, CREATE_TIME, UPDATE_TIME FROM OUYUNC_IM_FRIEND f WHERE f.USER_ID = :user_id AND EXISTS (SELECT 1 FROM OUYUNC_IM_USER u WHERE u.ID = f.USER_ID AND u.APP_KEY = :app_key AND u.DELETED = 0)", "按租户查询用户全部好友"),
        ...
        SELECT_ALL_GROUP_USER("SELECT ID, GROUP_ID, GROUP_CODE, GROUP_NICK_NAME, USER_ID, USER_CODE, POST, USER_NICK_NAME, SHIELD, SILENCE, WAY, CHANNEL, CREATE_TIME, JOIN_TIME FROM OUYUNC_IM_GROUP_USER gu WHERE gu.GROUP_ID = :group_id AND EXISTS (SELECT 1 FROM OUYUNC_IM_GROUP g WHERE g.ID = gu.GROUP_ID AND g.APP_KEY = :app_key AND g.DELETED = 0) AND ROWNUM <= :limit", "按租户查询群成员（须带 :limit，回源用上限+1）"),
