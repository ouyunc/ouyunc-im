...
import static com.ouyunc.base.utils.MqttCodecUtil.isValidClientId;
 ...
 */
public final class MqttEncoderUtil  {

    public static final MqttEncoderUtil INSTANCE = new MqttEncoderUtil();

    private MqttEncoderUtil() { }
    ...
    public  ByteBuf doEncode(MqttVersion mqttVersion,
        ...
        switch (message.fixedHeader().messageType()) {
            ...
            case CONNACK:
                return encodeConnAckMessage(mqttVersion, (MqttConnAckMessage) message);

            case PUBLISH:
                return encodePublishMessage(mqttVersion, (MqttPublishMessage) message);

            case SUBSCRIBE:
                return encodeSubscribeMessage(mqttVersion, (MqttSubscribeMessage) message);

            case UNSUBSCRIBE:
                return encodeUnsubscribeMessage(mqttVersion,  (MqttUnsubscribeMessage) message);

            case SUBACK:
                return encodeSubAckMessage(mqttVersion, (MqttSubAckMessage) message);

            case UNSUBACK:
                if (message instanceof MqttUnsubAckMessage) {
                    return encodeUnsubAckMessage(mqttVersion, (MqttUnsubAckMessage) message);
                }
                return encodeMessageWithOnlySingleByteFixedHeaderAndMessageId(ByteBufAllocator.DEFAULT, message);

            case PUBACK:
            case PUBREC:
            case PUBREL:
            case PUBCOMP:
                return encodePubReplyMessage(mqttVersion, message);

            case DISCONNECT:
            case AUTH:
                return encodeReasonCodePlusPropertiesMessage(mqttVersion, message);

            case PINGREQ:
            case PINGRESP:
                return encodeMessageWithOnlySingleByteFixedHeader(ByteBufAllocator.DEFAULT, message);
    ...
    private static ByteBuf encodeConnectMessage(
        ...
        try {
            ...
            if (variableHeader.isWillFlag()) {
                willPropertiesBuf = encodePropertiesIfNeeded(mqttVersion, ByteBufAllocator.DEFAULT, payload.willProperties());
            ...
            try {
                ...
                ByteBuf buf = ByteBufAllocator.DEFAULT.buffer(fixedHeaderBufferSize + variablePartSize);
    ...
    private static ByteBuf encodeConnAckMessage(
        ...
        try {
            ByteBuf buf = ByteBufAllocator.DEFAULT.buffer(4 + propertiesBuf.readableBytes());
    ...
    private static ByteBuf encodePubReplyMessage(MqttVersion mqttVersion,
                                          MqttMessage message) {
        if (message.variableHeader() instanceof MqttPubReplyMessageVariableHeader) {
            ...
            if (mqttVersion == MqttVersion.MQTT_5 &&
                ...
                propertiesBuf = encodeProperties(ByteBufAllocator.DEFAULT, variableHeader.properties());
            ...
            try {
                ...
                ByteBuf buf = ByteBufAllocator.DEFAULT.buffer(fixedHeaderBufferSize + variableHeaderBufferSize);
                buf.writeByte(getFixedHeaderByte1(mqttFixedHeader));
