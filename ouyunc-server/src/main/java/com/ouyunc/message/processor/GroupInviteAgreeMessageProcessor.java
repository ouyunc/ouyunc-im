 ...
 * 被邀请人同意加 群
 */
public final class GroupInviteAgreeMessageProcessor extends AbstractMessageProcessor<Byte> {
    private static final Logger log = LoggerFactory.getLogger(GroupInviteAgreeMessageProcessor.class);

    @Override
    public MessageType type() {
        return MessageTypeEnum.GROUP_REQUEST_INVITE_AGREE;
    }
    ...
    public void process(ChannelHandlerContext ctx, Packet packet) {
        if (log.isDebugEnabled()) {
            log.debug("GroupInviteAgreeMessageProcessor 正在处理外部客户端被邀请加群 {} ...", packet);
        }
        // 1. 保存消息
        ...
        try {
            if (lock.tryLock(MessageConstant.LOCK_WAIT_TIME, MessageConstant.LOCK_LEASE_TIME, TimeUnit.SECONDS)) {
                ...
                log.error("Failed to lock user invite agree group request message: {}", packet);
                MessageServerContext.publishEvent(new ExceptionEvent(ExceptionCodeEnum.ACQUIRE_LOCK_ERROR, "同意群邀请请求加锁失败", packet), true);
            }
        } catch (Exception e) {
            log.error("Failed to handle user invite agree group request message: {}", e.getMessage());
