 ...
 * 被邀请人拒绝加 群
 */
public final class GroupInviteRefuseMessageProcessor extends AbstractMessageProcessor<Byte> {
    private static final Logger log = LoggerFactory.getLogger(GroupInviteRefuseMessageProcessor.class);

    @Override
    public MessageType type() {
        return MessageTypeEnum.GROUP_REQUEST_INVITE_REFUSE;
    }
    ...
    public void process(ChannelHandlerContext ctx, Packet packet) {
        if (log.isDebugEnabled()) {
            log.debug("GroupInviteRefuseMessageProcessor 正在处理外部客户端被邀请加群 {} ...", packet);
        }
        // 1. 保存消息
        ...
        try {
            if (lock.tryLock(MessageConstant.LOCK_WAIT_TIME, MessageConstant.LOCK_LEASE_TIME, TimeUnit.SECONDS)) {
                ...
                if (GroupRequestSessionWay.INVITED.equals(way) && !GroupJoinerProcessStatus.PENDING.value().equals(groupRequestSession.getJoinerProcessStatus())) {
                    log.error("被邀请人 {} 已经处理", groupRequestSession.getJoiner());
                    return;
                }
                ...
                // 如果移除成功说明自己已经在该群且是群主或者群管理员
                if (groupMannerOrLeaderUsersIdentitySet.remove(message.getFrom())) {
                    log.error("{}, 已经在该群且是群主或者群管理员", message.getFrom());
                    return;
                }
                // 设置同意被邀请状态
                ...
                repository().savePacket2Mq(MqConstant.KAFKA_GROUP_REQUEST_TOPIC, packet.getMessage().getTo(), packet).whenComplete((result, ex) -> {
                    if (ex != null) {
                        log.error("拒绝邀请加群请求，发送mq异常，原因：{}", ex.getMessage());
                ...
                log.error("Failed to lock user invite refuse group request message: {}", packet);
                MessageServerContext.publishEvent(new ExceptionEvent(ExceptionCodeEnum.ACQUIRE_LOCK_ERROR, "拒绝群邀请请求加锁失败", packet), true);
            }
        } catch (Exception e) {
            log.error("Failed to handle user invite refuse group request message: {}", e.getMessage());
