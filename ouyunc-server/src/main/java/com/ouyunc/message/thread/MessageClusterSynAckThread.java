...
public class MessageClusterSynAckThread implements Runnable{
    ...
    public void run() {
        ...
        if (MessageServerContext.serverProperties().isClusterSplitBrainDetectionEnable()
                && ChronoUnit.MINUTES.between(beginTime, Instant.now()) >= MessageServerContext.serverProperties().getClusterSplitBrainDetectionDelayTime()
                && isSplitBrain(availableGlobalServer)) {
    ...
    private static boolean isSplitBrain(Set<Map.Entry<String, ChannelPool>> availableGlobalServer) {
        ClusterTopologyView topologyView = MessageServerContext.clusterTopologyView;
        if (topologyView.isZoneAware()) {
            int configuredInZone = topologyView.getLocalZoneConfiguredNodeCount();
            if (configuredInZone <= 1) {
                return false;
            }
            Set<String> activeRemoteNodes = MessageServerContext.clusterActiveServerRegistryTableCache.asMap().keySet();
            int activeInZone = topologyView.countActiveNodesInLocalZone(activeRemoteNodes);
            return activeInZone <= configuredInZone / 2;
        }
        return (MessageServerContext.clusterActiveServerRegistryTableCache.sizeMap() + NumberConstant.NUMBER_1) <= availableGlobalServer.size() / 2;
    }
}
